...
public class CompactMultiValueFacetHandler extends FacetHandler<FacetDataCache>
    implements FacetScoreable {
  private static Logger logger = Logger
      .getLogger(CompactMultiValueFacetHandler.class);
  ...
  public FacetCountCollectorSource getFacetCountCollectorSource(
      final BrowseSelection sel, final FacetSpec ospec) {
    return new FacetCountCollectorSource() {

      @Override
      public FacetCountCollector getFacetCountCollector(BoboIndexReader reader,
          int docBase) {
        final FacetDataCache dataCache = CompactMultiValueFacetHandler.this
            .getFacetData(reader);
        return new CompactMultiValueFacetCountCollector(_name, sel, dataCache,
            docBase, ospec);
      }
    };
  }
  ...
  public FacetDataCache load(BoboIndexReader reader) throws IOException {
    ...
    TermValueList mterms = _termListFactory == null ? new TermStringList()
        : _termListFactory.createTermList();
    ...
    while ((termBytes = termsEnum.next()) != null) {
      ...
      // freqList.add(termEnum.docFreq()); // removed because the df doesn't
      // take into account the num of deletedDocs
      int df = 0;
      int minID = -1;
      int maxID = -1;

      docEnum = termsEnum.docs(reader.getLiveDocs(), docEnum);

      if (docEnum != null) {
        int docid;
        while ((docid = docEnum.nextDoc()) != DocIdSetIterator.NO_MORE_DOCS) {
          df++;
    ...
    return new FacetDataCache(order, mterms, freqList.toIntArray(),
        minIDList.toIntArray(), maxIDList.toIntArray(), TermCountSize.large);
  }

  private static class CompactMultiFacetDocComparatorSource extends
      DocComparatorSource {
    private final CompactMultiValueFacetHandler _facetHandler;

    private CompactMultiFacetDocComparatorSource(
        CompactMultiValueFacetHandler facetHandler) {
    ...
    public DocComparator getComparator(final IndexReader reader, int docbase)
      ...
      if (!(reader instanceof BoboIndexReader))
        throw new IllegalStateException("reader must be instance of "
            + BoboIndexReader.class);
      final FacetDataCache dataCache = _facetHandler
      ...
      return new DocComparator() {
        ...
        public Comparable value(ScoreDoc doc) {
          return new StringArrayComparator(_facetHandler.getFieldValues(
              (BoboIndexReader) reader, doc.doc));
        }
  ...
  public BoboDocScorer getDocScorer(BoboIndexReader reader,
    ...
    FacetDataCache dataCache = getFacetData(reader);
    float[] boostList = BoboDocScorer.buildBoostList(dataCache.valArray,
        boostMap);
    return new CompactMultiValueDocScorer(dataCache, scoringFunctionFactory,
        boostList);
  }
  ...
  private static final class CompactMultiValueDocScorer extends BoboDocScorer {
    ...
    CompactMultiValueDocScorer(FacetDataCache dataCache,
      ...
      super(scoreFunctionFactory.getFacetTermScoringFunction(
          dataCache.valArray.size(), dataCache.orderArray.size()), boostList);
    ...
    public Explanation explain(int doc) {
      ...
      ArrayList<Explanation> explList = new ArrayList<Explanation>(
          scoreList.size());
      while (encoded != 0) {
        if ((encoded & 0x00000001) != 0x0) {
          int idx = count - 1;
          scoreList
              .add(_function.score(_dataCache.freqs[idx], _boostList[idx]));
          explList.add(_function
              .explain(_dataCache.freqs[idx], _boostList[idx]));
        }
        count++;
        encoded >>>= 1;
      }
